...
- **node_modules**: This directory houses our project's external dependencies and libraries, ensuring precise version control as defined in the "package.json" file.
  ...
  - **src/**: This library contains multiple directories, each serving specific purposes, which are summarized below.
    ...
    - **logger**: This directory holds custom logger for printing console statements and logging info.

    - **steps**: This directory holds the code for custom steps with log statements and wait mechanism.
...
- **tests**: This directory houses the framework setup, end-to-end test files, API tests, and associated utilities. The tests are organized as:
  ...
  - **apps**: This is where the Xcenters (PC,BC,CC) folders reside.
  ...
  - **config**: This directory is dedicated to storing enviornment settings and user roles.
  ...
  - **utilities**: This directory holds utilities to use in tests like date and string utils.
