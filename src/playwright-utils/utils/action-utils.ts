...
import { getPage, wait } from './page-utils';
import {
  ActionOptions,
  CheckOptions,
  ClearOptions,
...
import { getDefaultLoadState } from '@loadstate';
...
export async function fill(input: string | Locator, value: string, options?: FillOptions): Promise<void> {
  ...
  // Try the normal fill first (fastest / preferred)
  try {
    await locator.fill(value, options);
    return;
  } catch (err) {
    // fallback to handle custom widgets / nested inputs (e.g., gw-InputWidget div wrappers)
    try {
      await locator.evaluate(
        (el: Element, v: string) => {
          const setValueAndDispatch = (elToSet: any, val: string) => {
            if ('value' in elToSet) {
              elToSet.value = val;
            } else if (elToSet.isContentEditable) {
              elToSet.textContent = val;
            } else {
              return false;
            }
            elToSet.dispatchEvent(new Event('input', { bubbles: true }));
            elToSet.dispatchEvent(new Event('change', { bubbles: true }));
            return true;
          };

          // If element itself is fillable
          if ((el as HTMLElement).tagName === 'INPUT' || (el as HTMLElement).tagName === 'TEXTAREA' || (el as HTMLElement).isContentEditable) {
            if (setValueAndDispatch(el, v)) return;
          }

          // Prefer explicit child input/textarea/contenteditable or role textbox
          const selectors = 'input, textarea, [contenteditable="true"], [role="textbox"], [role="searchbox"]';
          let child = (el as Element).querySelector(selectors) as any;
          if (!child) {
            // handle guidewire style where there is a wrapper with id ending _Input that contains the actual input
            const wrapper = (el as Element).querySelector('[id$="_Input"]');
            if (wrapper) child = wrapper.querySelector(selectors) as any;
          }
          if (child && setValueAndDispatch(child, v)) return;

          // last resort: try to set attribute value and dispatch on the first matching descendant
          const first = (el as Element).querySelector(selectors) as any;
          if (first && setValueAndDispatch(first, v)) return;

          throw new Error('No fillable/input element found inside the target locator');
        },
        value,
      );
      return;
    } catch (innerErr) {
      // Re-throw original error for context if fallback fails
      throw err;
    }
  }
}
