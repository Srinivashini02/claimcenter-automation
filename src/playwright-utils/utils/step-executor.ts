 ...
 */
export async function performStep<T>(options: StepConfig<T>): Promise<T | void> {
  let retryOptions: RetryOptions = { retries: 0, delay: 1 * 1000 };
  if (options.retryOptions) retryOptions = options.retryOptions;
  const { retries, delay } = retryOptions;
  let result: T;
  await test.step(options.description, async () => {
    for (let attempt = 1; attempt <= retries + 1; attempt++) {
      try {
        logger.info(`${options.description} (Attempt ${attempt})`);
        result = await options.action();
        if (options.assertFn) {
          await options.assertFn();
        }
        return result;
      } catch (error) {
        const ex = error as Error;
        logger.error(`ERROR: ${options.description} - (attempt ${attempt}) \n message - ${ex.message}`);
        if (attempt > retries) {
          throw new Error(
            `ERROR: ${options.description} - (attempt ${attempt}) \n message - ${ex.message}\nstacktrace - ${ex.stack}`,
          );
        }
        await wait(delay);
      }
