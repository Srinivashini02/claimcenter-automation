...
import { ExpectOptions, ExpectTextOptions, SoftOption, TimeoutOption } from '@parameter-types';
 ...
 * @param {ExpectOptions} options - The options to pass to the expect function.
 */
export async function expectElementNotToHaveValue(
  input: string | Locator,
  text: string | RegExp,
  options?: ExpectOptions,
): Promise<void> {
  const { locator, assert } = getLocatorAndAssert(input, options);
  await assert(locator, options).not.toHaveValue(text, options);
}
